/**
 * scripts/remove-pure-utils.mjs
 * 删除 level-editor.js 中已迁移到以下模块的函数定义：
 *   - editor/cutscene-utils.js
 *   - editor/fetch-utils.js
 *   - editor/display-utils.js（normalizeCityIdentity, pickPreferredGameplayTab）
 */
import fs from 'fs';
...
const TARGETS = new Set([
    'projectPathFromVideoPublicUrl',
    'effectiveCutsceneVideoProjectPath',
    'formatIntroVideoStatusLines',
    'parseFetchErrorBody',
    'normalizeCityIdentity',
    'pickPreferredGameplayTab',
]);
...
while (idx < lines.length) {
    ...
    if (m && TARGETS.has(m[1])) {
        ...
        for (let j = idx - 1; j >= Math.max(0, idx - 15); j--) {
            const prev = lines[j];
            if (/^\s*$/.test(prev) || /^\s{4}(\s*\*|\/\*\*)/.test(prev) || /^\s{4}\/\//.test(prev) || /^\s{4}\*\//.test(prev) || /^\s*\/\/ ─/.test(prev)) {
                commentStart = j;
            } else break;
        }
