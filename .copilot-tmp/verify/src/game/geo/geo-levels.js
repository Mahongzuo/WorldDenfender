...
let geoSourcePromise = null;
export async function hydrateEditorLevelGeo(levels) {
...
function loadGeoSources() {
    geoSourcePromise ?? (geoSourcePromise = Promise.all([loadCountryGeo(), loadChinaCityGeo()])
...
async function loadCountryGeo() {
    const [data, remoteCapitals] = await Promise.all([
        fetchGeoJsonScript("/Web/data/world-data.js", "__WORLD_GEOJSON__"),
...
async function loadChinaCityGeo() {
    const china = await fetchGeoJsonScript("/Web/data/china-data.js", "__CHINA_GEOJSON__");
...
async function loadProvinceCityGeo(adcode) {
    try {
        const variableName = `__PROVINCE_${adcode}_GEOJSON__`;
        const data = await fetchGeoJsonScript(`/Web/data/provinces/${adcode}-data.js`, variableName);
        return (data.features ?? [])
            ...
            const entries = [[String(props.adcode || ""), geo]];
...
async function fetchGeoJsonScript(url, variableName) {
    ...
    return JSON.parse(text.slice(bodyStart, end > bodyStart ? end : undefined).trim());
}
function resolveChinaCityGeo(level, cities) {
...
function resolveCountryGeo(level, countries) {
    const candidates = collectIso3166Alpha3Candidates(String(level.id || ""), String(level.location?.countryCode || ""));
...
function createGeoConfig(point, extentMeters) {
    return {
        enabled: true,
        provider: "cesium-ion",
