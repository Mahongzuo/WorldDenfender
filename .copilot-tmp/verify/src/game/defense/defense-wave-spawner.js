...
import { clampDefenseDifficulty, getDefenseDifficultyRuntimeParams, } from "./defense-difficulty";
...
export function advanceDefenseSpawnState(input) {
...
function sanitizeEnemyTypeId(value) {
    if (!value)
        return undefined;
    switch (value) {
        case "basic":
    ...
    if (lower.includes("scout") || lower.includes("fast") || lower.includes("fly") || lower.includes("drone-fast"))
        return "scout";
    if (lower.includes("jammer") || lower.includes("hack") || lower.includes("signal") || lower.includes("caster"))
        return "hacker";
    if (lower.includes("heavy") || lower.includes("tank") || lower.includes("shield") || lower.includes("wall"))
        return "tank";
    if (lower.includes("swarm") || lower.includes("horde") || lower.includes("group") || lower.includes("cluster"))
        return "swarm";
    if (lower.includes("drone") || lower.includes("basic") || lower.includes("grunt") || lower.includes("soldier"))
        return "basic";
    return undefined;
}
function clampWaveRule(raw) {
    return {
        ...raw,
...
export function getDefenseWaveRulesForWave(rules, wave) {
    if (!rules?.length)
        return [];
    return rules
        .map(clampWaveRule)
...
function selectDefenseWaveRuleForOrdinal(rules, ordinal) {
    let seen = 0;
...
function orientPathCellsFromSpawn(cells, spawn) {
    if (!cells.length || !spawn)
        return cells.slice();
...
function orderedPathWorldPoints(cells, spawn, map) {
    if (!spawn) {
    ...
    const goal = map.path?.length >= 1 ? map.path[map.path.length - 1] : cells[cells.length - 1];
...
function resolveEnemyPath(map, rule) {
    ...
    const spawn = spawnById
        ...
        ?? map.spawnPoints?.[0];
    const path = pathId
        ? map.enemyPaths?.find((candidate) => candidate.id === pathId && candidate.cells.length)
        : undefined;
...
function findDefenseEnemyConfig(map, enemyTypeId) {
...
function applyRuntimeEnemyConfig(enemy, config) {
    if (!config) {
        return;
    }
    if (config.name) {
...
export function spawnDefenseWaveEnemy(deps) {
    ...
    const resolvedEnemyType = resolvedPath.enemyType || sanitizeEnemyTypeId(authoredEnemyConfig?.id) || sanitizeEnemyTypeId(authoredEnemyConfig?.name) || "basic";
