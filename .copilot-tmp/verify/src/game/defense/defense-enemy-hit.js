...
export function applyDefenseEnemyDamage(ctx, enemy, damage) {
    if (enemy.hp <= 0) {
        return;
    }
    enemy.hp -= damage;
    ...
    const healthMaterial = enemy.healthBar.material;
        ...
        for (const b of ctx.buildings) {
            if (b.spec.role === "ranged" || b.spec.role === "mage" || b.spec.role === "support") {
                if (distanceXZ(b.mesh.position, enemy.mesh.position) <= 4 * TILE_SIZE) {
                    applyDefenseEffectToBuilding(b, { statusId: "electromagneticInterference", duration: 3, magnitude: 3, element: "electric" }, ctx.elapsed);
    ...
    if (enemy.type === "swarm") {
        for (let i = 0; i < 3; i++) {
            ...
            const healthBar = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.12, 0.1), new THREE.MeshBasicMaterial({ color: 0x34ff6a, depthTest: false, depthWrite: false }));
            healthBar.renderOrder = 30;
            healthBar.userData.isEnemyHealthBar = true;
            const healthBarBack = new THREE.Mesh(new THREE.BoxGeometry(1.02, 0.18, 0.11), new THREE.MeshBasicMaterial({ color: 0x10131a, depthTest: false, depthWrite: false }));
