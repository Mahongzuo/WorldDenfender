...
import { parseEnemyArchetypeOverride, resetDefenseEnemyArchetypeOverrides, setDefenseEnemyArchetypeOverrides, } from "../defense/defense-enemy-archetypes";
import { sanitizeDefenseElement, sanitizeDefenseFunctionTags, sanitizeDefenseStatusIds, resetDefenseConsumableOverrides, setDefenseConsumableOverrides, } from "../defense/defense-taxonomy";
export function normalizeGameplayIdentity(value) {
    return String(value || "")
        .trim()
        .toLowerCase()
        .replace(/[·\s]/g, "")
...
export function applyNumericOverride(spec, key, value) {
    const numeric = Number(value);
    if (Number.isFinite(numeric)) {
        spec[key] = numeric;
    }
}
function clampDefenseEffectDurationSec(raw) {
    const v = Number(raw);
    if (!Number.isFinite(v) || v <= 0)
        return 2;
    return Math.round(Math.min(120, Math.max(0.1, v)) * 10) / 10;
}
function defenseEffectsFromStatusIds(raw, durationSec) {
...
function parseDefenseConsumableEntry(raw) {
...
export function resetBundledMapsBeforeEditorSync(bundledCityMapJson) {
    MAPS.length = MAPS_BUILTIN_COUNT;
    EXPLORE_MAPS.length = EXPLORE_MAPS_BUILTIN_COUNT;
    let restored;
    try {
        restored = JSON.parse(bundledCityMapJson);
    }
    catch {
        return;
    }
...
export function mergeTowerGameplayConfigsFromCityPayload(rawConfigs, ctx) {
    try {
        const restored = JSON.parse(ctx.bundledBuildSpecsJson);
        for (const key of Object.keys(BUILD_SPECS)) {
    ...
    const desiredCity = new Set([ctx.currentCity, ctx.currentCityLabel, ctx.requestedRegionCode, ctx.requestedRegionName]
    ...
    for (const [configKey, rawConfig] of Object.entries(rawConfigs)) {
        ...
        for (const tower of config.towers ?? []) {
            ...
            if (towerDesc)
                spec.description = towerDesc;
            const assetRefs = towerRaw.assetRefs && typeof towerRaw.assetRefs === "object" ? towerRaw.assetRefs : {};
            const rawModelScale = Number(assetRefs.modelScale);
            if (Number.isFinite(rawModelScale) && rawModelScale > 0)
                spec.modelScale = rawModelScale;
        }
        for (const enemy of config.enemies ?? []) {
            const type = String(enemy.id || "");
        ...
        if (Object.keys(enemyOverrides).length)
            setDefenseEnemyArchetypeOverrides(enemyOverrides);
        if (consumables.length)
            setDefenseConsumableOverrides(consumables);
    }
}
export function applyRequestedRegionToLevels(levels, requestedLevelId, requestedRegionCode, requestedRegionName) {
    ...
    if (isDefaultLevel) {
        const regionLabel = location.regionLabel ||
            location.cityName ||
...
export function createRequestedRegionLocation(requestedRegionCode, requestedRegionName) {
...
async function fetchGameModelPublicUrlByCatalogId() {
    try {
...
export async function pullEditorLevelsFromProjectFile(options) {
    ...
    applyRequestedRegionToLevels(levels, options.levelsPullCtx.requestedLevelId, options.towerOverrideCtx.requestedRegionCode, options.towerOverrideCtx.requestedRegionName);
