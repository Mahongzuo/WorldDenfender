...
export function advanceExploreDropTimer(current, dt, respawnIntervalSec, onSpawnBatch) {
    let next = current - dt;
    if (next <= 0) {
        onSpawnBatch();
...
export function tickExploreSafeZone(options) {
    const nowInSafe = options.safeZoneCells.size > 0 &&
    ...
    if (nowInSafe !== options.wasInSafeZone) {
        if (nowInSafe) {
            ...
            options.showToast("\u8fdb\u5165\u5b89\u5168\u533a \u2014 \u53ef\u5728\u5de6\u4e0b\u89d2\u5546\u5e97\u8d2d\u4e70\u8865\u7ed9");
        }
        else {
