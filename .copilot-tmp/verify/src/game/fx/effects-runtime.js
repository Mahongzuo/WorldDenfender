...
function makeProjectileMesh(buildId, color, fxGroup) {
...
function addMachineHitFx(effects, fxGroup, center, color) {
    const count = 18;
    ...
    const flash = new THREE.Mesh(new THREE.SphereGeometry(0.1, 6, 4), new THREE.MeshBasicMaterial({
...
function addFrostHitFx(effects, fxGroup, center, color) {
    const count = 24;
    const positions = new Float32Array(count * 3);
    const velocities = new Float32Array(count * 3);
    for (let i = 0; i < count; i++) {
...
function addStellarHitFx(effects, fxGroup, center, color) {
    const starPts = 8;
    const baseY = center.y + 0.06;
    for (let i = 0; i < starPts; i++) {
        const angle = (i / starPts) * Math.PI * 2;
        const length = 0.55 + Math.random() * 0.4;
        const endPt = new THREE.Vector3(center.x + Math.cos(angle) * length, baseY, center.z + Math.sin(angle) * length);
        const geo = new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(center.x, baseY, center.z), endPt]);
        const line = new THREE.Line(geo, new THREE.LineBasicMaterial({ color: blendTowardWhite(color, 0.15), transparent: true, blending: THREE.AdditiveBlending }));
        fxGroup.add(line);
    ...
    const flash = new THREE.Mesh(new THREE.SphereGeometry(0.15, 8, 6), new THREE.MeshBasicMaterial({
...
function addLiqingzhaoHitFx(effects, fxGroup, center, color) {
    const lineCount = 5;
    for (let i = 0; i < lineCount; i++) {
        const angle = (i / lineCount) * Math.PI * 2 + Math.random() * 0.6;
        const len = 0.45 + Math.random() * 0.65;
        const endPt = new THREE.Vector3(center.x + Math.cos(angle) * len, center.y + Math.random() * 0.3, center.z + Math.sin(angle) * len);
        const geo = new THREE.BufferGeometry().setFromPoints([
            new THREE.Vector3(center.x, center.y + 0.15, center.z),
            endPt,
        ]);
        const line = new THREE.Line(geo, new THREE.LineBasicMaterial({ color: blendTowardWhite(color, 0.12), transparent: true, blending: THREE.AdditiveBlending }));
        fxGroup.add(line);
    ...
    const flash = new THREE.Mesh(new THREE.SphereGeometry(0.14, 8, 6), new THREE.MeshBasicMaterial({
        color: 0x00e5ff,
        transparent: true,
        blending: THREE.AdditiveBlending,
        depthWrite: false,
...
function addPointHitBurst(effects, fxGroup, center, color, towerId) {
 ...
 */
export function addTowerProjectileImpactFx(effects, fxGroup, from, to, towerColor, towerId, splashImpactRadiusWorld) {
    ...
    group.add(core);
    const trail = new THREE.Points(new THREE.BufferGeometry().setAttribute("position", new THREE.BufferAttribute(new Float32Array([0, 0, 0]), 3)), new THREE.PointsMaterial({
    ...
    const flightEffect = {
        ...
        tick: (dt) => {
            ...
            group.position.set(THREE.MathUtils.lerp(fromC.x, toC.x, p), fromC.y + (toC.y - fromC.y) * p + yArc, THREE.MathUtils.lerp(fromC.z, toC.z, p));
...
export function addBeamEffect(effects, fxGroup, from, to, color, lineOpacity = 0.82, ttl = 0.12) {
    const fromL = worldPointToFxGroupLocal(fxGroup, from);
    const toL = worldPointToFxGroupLocal(fxGroup, to);
    ...
    if (length > 0.001) {
        const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, length, 8), new THREE.MeshBasicMaterial({
...
function addSparkBurstImpact(effects, fxGroup, center, color, impactScale = 1) {
    ...
    const flash = new THREE.Mesh(new THREE.SphereGeometry(0.14 * sc, 8, 6), new THREE.MeshBasicMaterial({
        color: blendTowardWhite(color, 0.22),
        transparent: true,
        blending: THREE.AdditiveBlending,
        depthWrite: false,
...
export function addAuroraLaserEffect(effects, fxGroup, from, to) {
    const fromL = worldPointToFxGroupLocal(fxGroup, from);
    const toL = worldPointToFxGroupLocal(fxGroup, to);
    const group = new THREE.Group();
    ...
    const core = new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.18, length > 1e-5 ? length : 0.01), new THREE.MeshBasicMaterial({ color: 0x7ff8ff, transparent: true, opacity: 0.42 }));
...
export function addExplosionEffect(effects, fxGroup, center, radius, color) {
...
export function addMuzzleFlashEffect(effects, fxGroup, position, color = 0xffaa00) {
    ...
    const sfxOk = sfx > 1.001 && Math.abs(sfx - fxGroup.scale.z) < 0.02 && fxGroup.scale.y <= 1.05;
    sprite.scale.set(0.8 * (sfxOk ? 1 / sfx : 1), 0.8, 1);
    fxGroup.add(sprite);
    effects.push({ object: sprite, ttl: 0.08, maxTtl: 0.08, grow: 1.5 });
}
export function addTrailEffect(effects, fxGroup, from, to, color = 0xffffff, lineOpacity = 0.42) {
...
export function addStatusOutlineEffect(effects, fxGroup, target, color, ttl = 0.5) {
    const group = new THREE.Group();
...
function pruneFxGroupOrphans(fxGroup, effects) {
    const keep = new Set();
...
export function updateTimedEffects(effects, fxGroup, dt) {
    const nextEffects = [];
    /**
