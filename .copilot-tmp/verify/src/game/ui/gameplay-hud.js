...
import { formatDefenseDifficultyHud } from "../defense/defense-difficulty";
export function refreshGameplayHud(dom, m) {
    const towerHudName = (id) => m.defenseTowerNameOverrides?.[id] ?? BUILD_SPECS[id].name;
    ...
    const awaiting = !!(m.defenseVictoryAwaitingChoice && m.mode === "defense");
    let waveText;
    if (awaiting) {
    ...
    const diffHud = m.defenseDifficultyTier != null ? ` \u00b7 ${formatDefenseDifficultyHud(m.defenseDifficultyTier)}` : "";
    ...
    dom.selectedBuildSummaryElement.textContent = `\u5f53\u524d\uff1a${towerHudName(m.selectedBuild)}\uff08$${spec.cost}\uff09${spec.requiresUnlock && !m.economy.sTowerUnlocked ? " \u00b7 \u672a\u89e3\u9501" : ""}`;
    ...
    if (m.selectedBuilding) {
        ...
        dom.selectedUnitStats.textContent = `\u751f\u547d\uff1a${Math.ceil(m.selectedBuilding.hp)}/${m.selectedBuilding.spec.maxHp ?? 1} | \u653b\u51fb\uff1a${m.selectedBuilding.spec.damage ?? 0}`;
        ...
        if (active) {
            ...
            if (cooldownSec > 0) {
                ...
                dom.activeSkillButton.setAttribute("title", `${active.description}\uff08\u5269\u4f59 ${cooldownSec}s\uff09`);
            }
            else {
                dom.activeSkillButton.textContent = `\u91ca\u653e ${active.name}`;
                dom.activeSkillButton.disabled = false;
                dom.activeSkillButton.setAttribute("title", `${active.description}\uff08\u6280\u80fd CD ${Math.round(active.cooldown)}s\uff09`);
            }
        }
        else {
    ...
    document.querySelectorAll(".build-button").forEach((button) => {
        ...
        const deployedUnique = buildSpec.rank === "S" && [...m.buildings].some((building) => building.spec.id === buildId);
        ...
        button.title = deployedUnique
            ...
            : "";
        const nameSpan = button.querySelector(".build-name > span:first-child");
        if (nameSpan) {
            nameSpan.textContent = `${buildSpec.key} \u00b7 ${towerHudName(buildId)}`;
        }
        const descDiv = button.querySelector(".build-desc");
        if (descDiv) {
...
export function refreshMapButtonsActiveState(activeIndex) {
    document.querySelectorAll(".map-button").forEach((button) => {
        button.classList.toggle("active", button.dataset.map === String(activeIndex));
    });
}
export function refreshBuildCardModelFlags(customModels) {
