...
export function createSpriteTowerMesh(spec) {
    const group = new THREE.Group();
    const color = spec.color;
    const base = new THREE.Mesh(new THREE.CylinderGeometry(0.62, 0.78, 0.15, 8), new THREE.MeshStandardMaterial({
        color: 0x1a2b3c,
        roughness: 0.68,
        emissive: color,
    ...
    const ring = new THREE.Mesh(new THREE.TorusGeometry(0.55, 0.03, 8, 32), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.5 }));
...
function createTowerPlaceholderTexture(colorNum) {
...
export function createCustomBuildingMesh(spec, source, scale) {
    const group = new THREE.Group();
    const base = new THREE.Mesh(new THREE.CylinderGeometry(0.72, 0.86, 0.2, 16), new THREE.MeshStandardMaterial({
        color: spec.color,
        emissive: spec.color,
    ...
    if (spec.range) {
        const ring = new THREE.Mesh(new THREE.TorusGeometry(spec.range * TILE_SIZE, 0.025, 8, 72), new THREE.MeshBasicMaterial({ color: spec.color, transparent: true, opacity: 0.35 }));
        ring.rotation.x = Math.PI / 2;
...
export function createTowerMesh(spec) {
    const group = new THREE.Group();
    const color = spec.color;
    ...
    if (spec.id === "stellar") {
        ...
        const halo = new THREE.Mesh(new THREE.TorusGeometry(0.78, 0.035, 8, 48), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.65 }));
...
export function createMineMesh(spec) {
    const group = new THREE.Group();
...
export function createBeaconMesh(spec) {
    const group = new THREE.Group();
    const material = new THREE.MeshStandardMaterial({
        color: spec.color,
        emissive: spec.color,
    ...
    const ring = new THREE.Mesh(new THREE.TorusGeometry((spec.buffRange ?? 3) * TILE_SIZE, 0.035, 8, 72), new THREE.MeshBasicMaterial({ color: spec.color, transparent: true, opacity: 0.45 }));
    ring.rotation.x = Math.PI / 2;
...
export function createFallbackPlayerMesh() {
    const group = new THREE.Group();
    const body = new THREE.Mesh(new THREE.CapsuleGeometry(0.34, 0.72, 5, 10), new THREE.MeshStandardMaterial({ color: 0x4bb6ff, roughness: 0.45 }));
    body.position.y = 0.62;
    body.castShadow = true;
    const pointer = new THREE.Mesh(new THREE.ConeGeometry(0.22, 0.45, 4), new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0x204c8f, emissiveIntensity: 0.25 }));
...
export function createMoneyDropMesh(options) {
    ...
    if (customDropModel) {
        const base = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.5, 0.12, 16), new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 0.12 }));
    ...
    const glow = new THREE.Mesh(new THREE.TorusGeometry(0.56, 0.035, 8, 28), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.45 }));
