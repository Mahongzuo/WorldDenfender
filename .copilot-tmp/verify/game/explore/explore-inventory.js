function isHealConsumable(item) {
...
export class ExploreInventory {
     ...
     */
    tryHealFromItem(idx, progress) {
        const item = this.items[idx];
        if (!item || item.quantity <= 0) {
            return { consumed: false, message: "" };
        }
        let used = false;
        const restore = item.name.includes("\u5f3a\u6548") || item.name.includes("\u5927") ? 150 : 50;
