/* ───── 全部技能模组定义（土豆兄弟式肉鸽升级池） ───── */
export const EXPLORE_SKILL_MODULES = [
    // ── 普攻 slot ──
    {
        id: "basic-rapid",
        name: "高频脉冲",
...
/** 根据模组等级计算实际数值倍率 */
export function resolveModuleStats(mod) {
...
export function aggregateSlotModules(equipped, slot) {
    let damageMult = 1;
    let cooldownMult = 1;
    let radiusMult = 1;
    const effectTags = [];
...
export function rollUpgradeChoices(equipped, count = 3) {
...
/** 选择模组后执行装备/升级 */
export function applyModuleChoice(equipped, moduleId) {
...
/** 检查被动效果是否存在 */
export function hasPassiveEffect(equipped, tag) {
    for (const eq of equipped) {
        const def = getModuleDef(eq.moduleId);
        if (def?.slot === "passive" && def.effectTags?.includes(tag))
            return true;
    }
    return false;
}
/** 获取被动减伤系数（0-1 之间，1 表示无减伤） */
export function getPassiveDamageReduction(equipped) {
    let reduction = 1;
    for (const eq of equipped) {
        const def = getModuleDef(eq.moduleId);
        if (!def || def.slot !== "passive" || !def.effectTags?.includes("damage-reduction"))
            continue;
...
/** 获取被动经验加成倍率 */
export function getPassiveXpMult(equipped) {
    let mult = 1;
    for (const eq of equipped) {
        const def = getModuleDef(eq.moduleId);
        if (!def || def.slot !== "passive" || !def.effectTags?.includes("xp-boost"))
            continue;
        const scaling = 1 + (def.levelScaling ?? 0) * (eq.level - 1);
        mult *= 1 + 0.3 * scaling;
    }
...
/** 获取被动暴击率 */
export function getPassiveCritChance(equipped) {
    let chance = 0;
    for (const eq of equipped) {
        const def = getModuleDef(eq.moduleId);
        if (!def || def.slot !== "passive" || !def.effectTags?.includes("crit"))
            continue;
        const scaling = 1 + (def.levelScaling ?? 0) * (eq.level - 1);
        chance += 0.15 * scaling;
    }
...
/** 获取被动移速加成倍率 */
export function getPassiveSpeedMult(equipped) {
    let mult = 1;
    for (const eq of equipped) {
        const def = getModuleDef(eq.moduleId);
        if (!def || def.slot !== "passive" || !def.effectTags?.includes("move-speed"))
            continue;
        const scaling = 1 + (def.levelScaling ?? 0) * (eq.level - 1);
        mult *= 1 + 0.25 * scaling;
    }
...
export function getPassiveRegenRatio(equipped) {
    let ratio = 0;
    for (const eq of equipped) {
        const def = getModuleDef(eq.moduleId);
        if (!def || def.slot !== "passive" || !def.effectTags?.includes("regen"))
            continue;
        const scaling = 1 + (def.levelScaling ?? 0) * (eq.level - 1);
        ratio += 0.01 * scaling;
    }
