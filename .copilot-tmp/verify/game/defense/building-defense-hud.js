...
export class BuildingDefenseHud {
    ...
    syncSkillHudScaleCorrection(buildings) {
    ...
    getVisualTopLocal(building, fallback) {
        ...
        const apexWorld = new THREE.Vector3((worldBox.min.x + worldBox.max.x) * 0.5, worldBox.max.y, (worldBox.min.z + worldBox.max.z) * 0.5);
    ...
    layoutHealthForBuilding(building) {
    ...
    attachToBuilding(building) {
        ...
        const background = new THREE.Mesh(new THREE.PlaneGeometry(1.34, 0.16), new THREE.MeshBasicMaterial({ color: 0x111827, depthTest: false, depthWrite: false }));
        const fill = new THREE.Mesh(new THREE.PlaneGeometry(1.18, 0.1), new THREE.MeshBasicMaterial({ color: 0x52ff7f, depthTest: false, depthWrite: false }));
    ...
    updateSkillHudForBuilding(building) {
        ...
        const material = building.skillHudPlane.material;
        material.map?.dispose();
        material.map = this.createHudTexture(text, ready);
        material.needsUpdate = true;
    }
    updatePerFrame(buildings) {
        ...
        for (const building of buildings) {
            ...
            if (building.healthBarGroup && building.healthBarFill) {
                ...
                if (showHealth) {
                    ...
                    const material = building.healthBarFill.material;
