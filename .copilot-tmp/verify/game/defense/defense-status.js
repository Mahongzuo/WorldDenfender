function upsertStatus(list, status) {
    const next = list ?? [];
...
export function applyDefenseEffectToEnemy(enemy, effect, elapsed) {
...
export function applyDefenseEffectToBuilding(building, effect, elapsed) {
    const duration = Math.max(0, Number(effect.duration) || 0);
    if (duration <= 0) {
        return;
    }
...
export function applyBuildingEffectsToEnemy(building, enemy, elapsed) {
